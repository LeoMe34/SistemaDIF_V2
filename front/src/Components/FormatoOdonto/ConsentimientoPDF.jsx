...
const generarPDF = (detallePaciente, noExpediente, data, nombreE, cedula, fechaActual) => {
    ...
    yPosition = addTextWithWrap(`FECHA ${fechaActual}`, 20, yPosition + 10, maxWidth);
    yPosition = checkAddPage(yPosition, 10);
    yPosition = addTextWithWrap(`DIAGNÓSTICO: ${data.diagnostico}`, 20, yPosition + 10, maxWidth);
    yPosition = checkAddPage(yPosition, 10);
    yPosition = addTextWithWrap(`PLAN: ${data.plan}`, 20, yPosition + 10, maxWidth);
    yPosition = checkAddPage(yPosition, 10);
    yPosition = addTextWithWrap(`ACEPTACIÓN DEL PACIENTE, FIRMA`, 20, yPosition + 10, maxWidth);

    // Agregar línea debajo de la aceptación del paciente
    const lineStartX = 20;
    const lineEndX = maxWidth + 20;
    const lineY = yPosition + 5; // Espacio después del texto
    documento.line(lineStartX, lineY, lineEndX, lineY);

    yPosition = lineY + 5; // Ajustar la posición y después de la línea

    yPosition = checkAddPage(yPosition, 10);
    yPosition = addTextWithWrap(`TRATAMIENTO: ${data.tratamiento}`, 20, yPosition + 10, maxWidth);
    yPosition = checkAddPage(yPosition, 10);
    ...
    yPosition = addTextWithWrap(`FECHA: ${fechaActual}`, 20, yPosition + 10, maxWidth);
    yPosition = checkAddPage(yPosition, 10);
    yPosition = addTextWithWrap(`NÚMERO DE EXPEDIENTE: ${noExpediente}`, 20, yPosition + 10, maxWidth);
    yPosition = checkAddPage(yPosition, 10);
    yPosition = addTextWithWrap(`NOMBRE: ${detallePaciente?.datosPersonalesPacient?.nombre} ${detallePaciente?.datosPersonalesPacient?.apellidoP} ${detallePaciente?.datosPersonalesPacient?.apellidoM}`, 20, yPosition + 10, maxWidth);
    yPosition = checkAddPage(yPosition, 10);
    yPosition = addTextWithWrap(`EDAD: ${detallePaciente?.datosPersonalesPacient?.edad}`, 20, yPosition + 10, maxWidth);
    yPosition = checkAddPage(yPosition, 10);
    yPosition = addTextWithWrap(`${data.resumen}`, 20, yPosition + 10, maxWidth);
    yPosition = checkAddPage(yPosition, 10);
    yPosition = addTextWithWrap(`DIAGNÓSTICO: ${data.diagnostico}`, 20, yPosition + 10, maxWidth);
    yPosition = checkAddPage(yPosition, 10);
    yPosition = addTextWithWrap(`TRATAMIENTO: ${data.tratamiento}`, 20, yPosition + 10, maxWidth);
    ...
    documento.save(`Consentimiento_y_Nota_Evolucion_${noExpediente}.pdf`);
};
