...
const GraficosAudEdad = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get('http://127.0.0.1:8000/api/get_graficosAudEdad/', {
                    params: { month, year }
                });
    ...
    const getDetallesFichas = async () => {
        try {
            ...
            const fichasMedico = await Promise.all(fichas.map(async ficha => {
                ...
                if (empleado.groups && empleado.groups.includes('audiologo')) {
                    return ficha;
                } else {
                    return null;
                }
...
export default GraficosAudEdad;
