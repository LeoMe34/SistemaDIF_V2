...
const GraficosOftEdad = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get('http://127.0.0.1:8000/api/get_graficosOftEdad/', {
                    params: { month, year }
                });
                ...
                console.error('Error en obtener los datos del grafico oftalmologo:', error);
            }
        };
        fetchData();
    }, [month, year]);
    ...
    useEffect(() => {
        ...
        if (chartType === 'bar') {
            option = {
                title: {
                    text: 'Distribución de edades de los pacientes',
                    left: 'center',
                },
                tooltip: {
                    trigger: 'axis',
    ...
    const getDetallesFichas = async () => {
        try {
            ...
            const fichasMedico = await Promise.all(fichas.map(async ficha => {
                ...
                if (empleado.groups && empleado.groups.includes('oftalmologo')) {
                    return ficha;
                } else {
                    return null;
                }
...
export default GraficosOftEdad;
